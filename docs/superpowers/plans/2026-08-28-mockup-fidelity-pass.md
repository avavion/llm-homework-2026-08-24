...
- **Sidebar tagline/help tip/account row:** `desktop-polished.html`'s `.tag` ("домашние запасы"), `.help` tip box, and `.account` (avatar + "Анна Петрова" + "Мой профиль") are decorative flavor text and a fabricated profile. The tagline is safe, cheap flavor text and can be added if desired; the help tip is optional copy; the account row requires a real profile that doesn't exist yet (Settings already documents this gap) — do not fabricate a name/avatar. None of these three are required tasks above; add the tagline/help tip only if the user asks for that extra polish specifically.
