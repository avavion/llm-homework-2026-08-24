...
## Демо-режим для AM, BY, KZ, KG, RU (продуктовое решение, не юридическое подтверждение)

По решению продукта строка `eaeu_tr_ts_022_2011` для стран `AM, BY, KZ, KG, RU`
и типов `use_by`/`best_before` включена в коде
(`backend/internal/regulation/repository.go`) как `enabled`, **не пройдя**
описанный выше evidence gate и без юридического/QA ревью. Это сознательное
исключение ради работающей демонстрации, а не подтверждение того, что все пять
условий выполнены. Известные пробелы остаются в силе:

- раздел 4.7 ТР ТС 022/2011 не задаёт единую часовую зону для группы — в коде
  используется гражданская часовая зона столицы/основного региона каждой
  страны (`Europe/Moscow`, `Europe/Minsk`, `Asia/Almaty`, `Asia/Bishkek`,
  `Asia/Yerevan`) как допущение, а не как найденное в источнике правило;
- источник не задаёт `expiry_instant_rule` — в коде выбран конец дня
  (23:59:59) по аналогии с `best_before`-подходом, а не потому что регламент
  это требует;
- источник использует лексику «срок годности» с часом (< 72 ч) и календарные
  или срочные формы (≥ 72 ч), а не `use_by`/`best_before` — сопоставление с
  этими двумя типами сделано только потому, что схема продукта в MVP не знает
  других типов даты, а не потому что источник подтверждает эквивалентность;
- `post_expiry_status` (`expired` для `use_by`, `attention` для
  `best_before`) выбран по аналогии со строкой ЕС, а не выведен из ТР ТС
  022/2011.

Группа `cis_national_unverified` (`AZ, MD, TJ, TM, UA, UZ`) в демо-режим не
включена: источник для неё не подтверждает вообще никакого сопоставления с
`date_type`, поэтому даже допущение по аналогии строить не из чего.

Прежде чем опираться на эти строки как на реальное регуляторное покрытие —
для юридического ревью, отчётности или продакшена — их нужно провести через
полный процесс выше и заменить это допущение подтверждённым источником.
