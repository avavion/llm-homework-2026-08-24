...
type ApiDraft = { id: string; status: 'pending' | 'approved' | 'rejected'; fields: DraftFields }
...
const toPayload = (input: ProductInput) => ({ name: input.name, date_type: input.dateType, expiry_date: input.expiryDate, product_group: input.group || undefined, storage_location: input.location || undefined })
...
export const api = {
  products: {
    ...
    create: async (input: ProductInput) => apiMode === 'fixture' ? fixtureApi.products.create(input) : fromApiProduct(await request<ApiProduct>('/v1/products', { method: 'POST', body: JSON.stringify(toPayload(input)) })),
  ...
  drafts: {
    recognize: (file: File) => apiMode === 'fixture' ? fixtureApi.drafts.recognize(file) : (() => { const form = new FormData(); form.append('image', file); form.append('locale', navigator.language); return request<ApiDraft>('/v1/product-drafts/recognize', { method: 'POST', body: form, headers: {} }) })(),
    get: (id: string) => apiMode === 'fixture' ? fixtureApi.drafts.get(id) : request<ApiDraft>(`/v1/product-drafts/${id}`),
    approve: (id: string, input: ProductInput) => apiMode === 'fixture' ? fixtureApi.drafts.approve(id, input) : request<ApiProduct>(`/v1/product-drafts/${id}/approve`, { method: 'POST', body: JSON.stringify(toPayload(input)) }).then(fromApiProduct),
